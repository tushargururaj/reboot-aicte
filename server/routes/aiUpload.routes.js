...
router.post('/process', (req, res, next) => {
    ...
    const upload = multer({
        storage: multer.memoryStorage(), // Use memory storage for direct processing
        limits: {
            fileSize: 50 * 1024 * 1024 // 50MB limit for memory upload
        },
        fileFilter: (req, file, cb) => {
            const allowedTypes = /jpeg|jpg|png|gif|pdf|bmp|tiff/;
            const extname = allowedTypes.test(path.extname(file.originalname).toLowerCase());
    ...
    router.post('/process', (req, res, next) => {
        ...
        try {
            ...
            // 2. Handle Multer Memory Upload (Fallback for small files)
                ...
                console.log('Processing Multer file (Memory):', req.file.originalname);
    ...
    router.post('/confirm', async (req, res) => {
        try {
            ...
            // No need to move files for GCS/Memory flow as they are already in place or handled
            // If it was a local file upload (legacy), we might need to move it, but we removed that logic.
            // For GCS, the file is already in the bucket.
            // For Memory upload, we don't have a persistent file to move unless we wrote it (which we don't want to).
